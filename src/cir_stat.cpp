...
arma::vec sph_stat_LSE(arma::cube X, double kappa, bool Psi_in_X, 
                       arma::uword p);
arma::vec sph_stat_Poisson(arma::cube X, double rho, bool Psi_in_X, 
                       arma::uword p);
...
//' @export
// [[Rcpp::export]]
arma::vec cir_stat_Poisson(arma::mat Theta, double rho = 0.5,
                       bool Psi_in_Theta = false) {
  ...
  if (Psi_in_Theta) {
    ...
    arma::cube Theta_cube(Theta.n_rows, Theta.n_cols, 1);
    Theta_cube.slice(0) = Theta;
    return sph_stat_Poisson(Theta_cube, rho, true, 2);
    
  } else {
    
    return sph_stat_Poisson(Theta_to_X(Theta), rho, Psi_in_Theta, 2);
